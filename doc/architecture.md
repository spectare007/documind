...
- **TorchDynamo is disabled** (`TORCHDYNAMO_DISABLE=1` in `backend/Dockerfile`). Docling's model path invokes `torch.compile`, which needs a C++ compiler the slim base image doesn't have; since this all runs CPU-only anyway, compilation buys no speedup, so it's turned off rather than adding a toolchain to the image. See `doc/design-decisions.md`.
